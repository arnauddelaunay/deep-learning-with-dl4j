...
import com.example.predictor.preprocessing.SentimentExampleIterator
...
object Predict {
  ...
  val test = new SentimentExampleIterator(Constants.DATA_PATH, wordVectors, 1, Constants.truncateReviewsToLength, false)

  val model: MultiLayerNetwork = ModelSerializer.restoreMultiLayerNetwork(Constants.modelsPath + Constants.modelVersion)
